...
| Phase 2: LLM + Runtime + Decision | 26 | 0 | ░░░░░░░░░░ 0% |
...
- [ ] **1.1** — Create `config/settings.yaml` with all app settings (models, interfaces, hardware limits)
  - [ ] 1.1.1 — Add Jarvis name, language, wake_word settings
  - [ ] 1.1.2 — Add model names that match Ollama pulls: `gemma3:4b` (fast), `qwen3:8b` (general / balanced / deep), `qwen2.5-coder:7b` (coder), `llava:7b` (vision)
  - [ ] 1.1.3 — Add interface ports and hosts
  - [ ] 1.1.4 — Add storage paths

- [ ] **1.2** — Create `config/models.yaml` with model-specific parameters
  - [ ] 1.2.1 — Temperature, top_p, max_tokens per model
  - [ ] 1.2.2 — Capability profiles (reasoning tier, Arabic quality, code bias, latency, VRAM estimate)
  - [ ] 1.2.3 — Optional routing weights / thresholds (tunable without code changes)

- [ ] **1.3** — Create `.env.example` with all required environment variables
  - [ ] 1.3.1 — TELEGRAM_BOT_TOKEN
  - [ ] 1.3.2 — GOOGLE_CLIENT_ID / SECRET
  - [ ] 1.3.3 — YOUTUBE_API_KEY

- [ ] **1.4** — Create `config/skills.yaml` — tool/skill registry (enable/disable, categories) and pointers to JSON schemas

- [ ] **1.5** — Setup `requirements.txt` — complete and pinned dependencies
  - [ ] 1.5.1 — Core: ollama, fastapi, uvicorn, pydantic
  - [ ] 1.5.2 — Memory: chromadb, redis, sqlite3
  - [ ] 1.5.3 — Audio: openai-whisper, piper-tts, openwakeword, pyaudio, sounddevice
  - [ ] 1.5.4 — Vision: diffusers, torch, torchvision, Pillow
  - [ ] 1.5.5 — Tools/skills: playwright, psutil, pyautogui, google-api-python-client
  - [ ] 1.5.6 — Interfaces: python-telegram-bot, rich, PyQt6
  - [ ] 1.5.7 — Utils: loguru, pyyaml, python-dotenv, sentence-transformers

- [ ] **1.6** — Create `app/main.py` — entry point with `--interface` argument parser
  - [ ] 1.6.1 — Parse CLI args: `--interface [cli|web|telegram|gui|all]`
  - [ ] 1.6.2 — Initialize logging with Loguru
  - [ ] 1.6.3 — Load config from settings.yaml
  - [ ] 1.6.4 — Launch selected interface(s)

- [ ] **1.7** — Setup installation scripts — full automated installation
  - [ ] 1.7.1 — `scripts/install.sh` — Linux/WSL: system packages, venv, pip, Playwright, Ollama model pulls
  - [ ] 1.7.2 — `scripts/install.ps1` — Windows: Ollama install, model pulls, venv, pip, Playwright
  - [ ] 1.7.3 — Whisper model download instructions
  - [ ] 1.7.4 — Piper voice download instructions
...
- [ ] **2.2** — Create `src/models/llm/router.py` — dynamic model selector
  ...
  - [ ] 2.2.4 — Prefer fast profile (`gemma3`) when complexity low + mode `fast` + latency tight
  - [ ] 2.2.5 — Prefer deep profile (`qwen3`) when complexity high or mode `deep` / escalation
  - [ ] 2.2.6 — Default balanced profile when no stronger signal wins
...
- [ ] **2.6** — Create `src/core/runtime/loop/` — Observe → Decide → Think → Act → Evaluate driver
  ...
  - [ ] 2.6.4 — Act: delegate to executor stub (no-op until Phase 5)
  - [ ] 2.6.5 — Evaluate: compute posterior confidence + answer quality
  - [ ] 2.6.6 — Branch: Finish OR Escalate (bounded)
  - [ ] 2.6.7 — Failure: surface errors as observations; fallback safe response
  - [ ] 2.6.8 — Context Buffer: Observe reads from buffer snapshot when active
...
- [ ] **2.11** — Create `src/core/runtime/decision/` — Decision Layer
  ...
  - [ ] 2.11.4 — Time / effort estimation
  - [ ] 2.11.5 — Tool necessity detection
  - [ ] 2.11.6 — Planning trigger
  - [ ] 2.11.7 — Prior confidence
  - [ ] 2.11.8 — Cost estimate
  - [ ] 2.11.9 — Context Buffer as input source for Decision
  - [ ] 2.11.10 — Identity priors from User profile

- [ ] **2.12** — Mode controller — bind Thinking Modes to prompt packs + decode params
  - [ ] 2.12.1 — Map fast | normal | deep | planning | research → template + parameter set
  - [ ] 2.12.2 — Allow same model to switch modes between turns

- [ ] **2.13** — Escalation engine
  - [ ] 2.13.1 — Triggers: low posterior, incomplete answer, tool failure
  - [ ] 2.13.2 — Actions: deeper mode, stronger model, enable planning
  - [ ] 2.13.3 — max_escalation_depth + max_iterations + per-step timeouts
  - [ ] 2.13.4 — Retry policy — bounded retries with backoff

- [ ] **2.14** — Capability profiles — structured metadata per model
  - [ ] 2.14.1 — Fields: reasoning strength, Arabic quality, latency tier, code bias, vision, VRAM estimate
  - [ ] 2.14.2 — Router reads profiles; weights/thresholds configurable

- [ ] **2.15** — Integrate Decision Layer ↔ router ↔ loop
  - [ ] 2.15.1 — Decision injects mode + flags before Think; router scores auto model selection
  - [ ] 2.15.2 — Mid-loop model switch when escalation fires
  - [ ] 2.15.3 — Context Buffer path: end-to-end interface → enqueue → Observe → Decide → Think

- [ ] **2.16** — Limits & fallback policy
  - [ ] 2.16.1 — Per-turn / per-tool / per-load timeouts
  - [ ] 2.16.2 — Safe fallback response when caps hit

- [ ] **2.17** — Test LLM + runtime + decision wiring
  - [ ] 2.17.1 — Arabic chat with adaptive mode selection
  - [ ] 2.17.2 — Code path: verify scoring favors coder profile
  - [ ] 2.17.3 — Deep escalation: low-confidence → deeper mode
  - [ ] 2.17.4 — Single-GPU constraint + mid-loop swap
  - [ ] 2.17.5 — DecisionOutput contract tests

- [ ] **2.18** — Confidence system — prior + posterior
  - [ ] 2.18.1 — Prior in Decision Layer
  - [ ] 2.18.2 — Posterior in Evaluate
  - [ ] 2.18.3 — Threshold bands (high/medium/low) as config

- [ ] **2.19** — Cost estimation system
  - [ ] 2.19.1 — Populate cost_estimate.tokens
  - [ ] 2.19.2 — Map model choice + mode → latency / gpu_load tiers
  - [ ] 2.19.3 — Router uses cost × quality tradeoff

- [ ] **2.20** — Evaluate stage — first-class module
  - [ ] 2.20.1 — Inputs: candidate answer, tool traces, DecisionOutput, memory hints
  - [ ] 2.20.2 — Outputs: quality score, posterior confidence, stop/escalate recommendation

- [ ] **2.21** — Memory-influenced decision — integrate with Phase 3
  - [ ] 2.21.1 — Feed user preferences + language bias into Decision
  - [ ] 2.21.2 — Track repeated patterns → adjust priors
  - [ ] 2.21.3 — Log failures/successes → bias tools/planning
  - [ ] 2.21.4 — Define stable interfaces so Phase 2 runs with stubs

- [ ] **2.22** — Integration test — confidence + cost + evaluate + escalation
  - [ ] 2.22.1 — Low prior → cheap route; high complexity → higher cost
  - [ ] 2.22.2 — Posterior low → escalation within depth limit
  - [ ] 2.22.3 — Limits hit → fallback path
...
- [ ] **3.4** — Create `src/core/memory/manager.py` — unified memory interface
  ...
  - [ ] 3.4.4 — Lazy initialization of backends (only connect when first needed)

- [ ] **3.5** — Integrate memory into orchestrator + runtime Observe step
  - [ ] 3.5.1 — Auto-inject relevant memories into LLM context
  - [ ] 3.5.2 — Auto-save each interaction after turn completion
...
- [ ] **3.7** — User profiling (adaptive memory) — `src/core/memory/user_profile.py`
  ...
  - [ ] 3.7.4 — JSON file storage keyed by user/session; versioned updates

- [ ] **3.8** — Memory-driven decisions — close the loop with Phase 2
  - [ ] 3.8.1 — Influence model selection (boost/penalize profiles per task pattern)
  - [ ] 3.8.2 — Influence thinking mode (bias toward fast vs deep from history)
  - [ ] 3.8.3 — Integration: memory.manager exposes hooks consumed by Decision (2.11)
...
- [ ] **3.10** — Input registration
  ...
  - [ ] 3.10.3 — Timestamp each entry (ordering, TTL, debugging)

- [ ] **3.11** — Lightweight preprocessing (preparation only)
  - [ ] 3.11.1 — Basic parsing for text: normalize whitespace, optional chunking
  - [ ] 3.11.2 — File metadata: size, extension, mime detection
  - [ ] 3.11.3 — Extract cheap metadata only (image dimensions, page count); no heavy models

- [ ] **3.12** — Buffer lifecycle
  - [ ] 3.12.1 — Clear after successful execute / turn completion
  - [ ] 3.12.2 — Persist for session: in-memory with optional backing
  - [ ] 3.12.3 — Optional timeout cleanup: evict stale items; configurable idle TTL

- [ ] **3.13** — Integration (mandatory wiring)
  - [ ] 3.13.1 — Runtime loop: Observe uses buffer per 2.6.8–2.6.10
  - [ ] 3.13.2 — Decision Layer: primary signals from merged buffer + 2.11.9
  - [ ] 3.13.3 — Tool system: Act phase resolves buffer refs to paths (5.16); heavy readers run only when tools require them
...
- [ ] **3.14** — User profile (identity-aware) — `src/core/identity/user_profile.py`
  ...
  - [ ] 3.14.4 — Load into runtime: available to Observe, Decision, prompt builder
  - [ ] 3.14.5 — Dynamic updates: CLI/Web commands edit profile

- [ ] **3.15** — Jarvis profile (system identity) — `config/jarvis_identity.yaml`
...
- [ ] **3.18** — Shared context awareness
  - [ ] 3.18.1 — Same conversation state: rolling summary + message window + tool traces
...
- [ ] **3.19** — File & system awareness (controlled) — `src/core/identity/system_awareness.py`
  ...
  - [ ] 3.19.4 — Explicit non-goals: do not expose raw system; redact secrets

- [ ] **3.20** — Integration (mandatory wiring)
  - [ ] 3.20.1 — Decision Layer: consume user expertise / goals as priors
  - [ ] 3.20.2 — Runtime loop: Observe merges buffer + memory + identity into prompt build request
  - [ ] 3.20.3 — Context Buffer: staged attachments described in task context section
  - [ ] 3.20.4 — Memory system: long-term facts + profile + adaptive unified read path
  - [ ] 3.20.5 — Engine: document contract for Phase 12 (Personality) as overlay
